	...
	Object.keys(rooms).forEach(function(index){
		...
		casper.each(rms, function(self, identifier){
			...
			casper.waitForSelector('#classlist ul#day0',function(){
			  classes = this.evaluate(function(dd){
				var cl = {};
				var id,currentClassDate,currentClassDateObj,linktext,isPrivate,enrolled,is60;
				var now = moment().subtract(2,'hours').tz(dd.timezone);
				var later = moment().add(dd.duration,'hours').tz(dd.timezone);
				...
				this.eachThen(cl, function(r){
					...
					this.then(function(){
					  ...
					  function then(){
							...
							this.waitForSelector('#roomlayout > #spotwrapper > #spotcell10 .spotcustomer',function(){
								casper.then(function(){
									...
									var header = "'spotID','spotLabel','status','customerId','customerName'\r\n";
										...
										jQuery('#spotwrapper > span').each(function(i,v){
											if(typeof jQuery(v) !== 'undefined'){
												if(jQuery(v).hasClass('booked')){
													...
													customerid = jQuery(v).find('.spotcustomer a').data('customerid');
